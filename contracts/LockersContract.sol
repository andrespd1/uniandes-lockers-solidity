...
x
    modifier soloUniandes(uint _index) {
        require(msg.sender == Locker_por_Num[_index].uniandes, "Solo la Universidad de los Andes tiene acceso a esta funcion");
        _;
    }

    modifier noUniandes(uint _index) {
        require(msg.sender != Locker_por_Num[_index].uniandes, "Solo los estudiantes tienen acceso a esta funcion");
        _;
    }
    ...
    modifier ExpiraContrato(uint _index) {
        ...
        require(now > time, "El contrato no ha expirado");
        _;
    }

    function anadirLocker(string memory _edificio, string memory _tamano, uint _precio_dia_dia) public {
        require(msg.sender != address(0));
        num_lockers ++;
        bool _libre = true;
        Locker_por_Num[num_lockers] = Locker(num_lockers,0,_edificio,_tamano,_precio_dia,0,_libre, msg.sender, address(0)); 
    ...
    function firmarContrato(uint _index, uint _duracion_contrato) public payable noUniandes(_index) fondosSuficientes(_index, _duracion_contrato) soloLibre(_index) {
        ...
        num_contratos++;
        Locker_por_Num[_index].estudianteActual = msg.sender;
        ...
        Contrato_por_Num[num_contratos]=ContratoLocker(_index,num_contratos,Locker_por_Num[_index].edificio,Locker_por_Num[_index].tamano,Locker_por_Num[_index].precio_dia,_duracion_contrato,block.timestamp,_uniandes,msg.sender);
    ...
    function entregarLocker(uint _index) public noUniandes(_index) {
        require(msg.sender != address(0));
        Locker_por_Num[_index].libre = true;
    }
}